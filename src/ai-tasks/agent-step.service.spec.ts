...
describe('AgentStepService', () => {
  ...
  it('writes assistant message and releases conversation when a task completes', async () => {
  ...
  it('releases conversation on final failure', async () => {
  ...
  it('keeps conversation busy for retryable failure', async () => {
    tx.aiTask.updateMany.mockResolvedValue({ count: 1 });
    tx.aiTask.findUnique.mockResolvedValue({
      id: 'task-1',
      conversationId: 'conv-1',
