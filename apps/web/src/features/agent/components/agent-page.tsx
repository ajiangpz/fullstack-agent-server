...
export function AgentPage() {
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    try {
      ...
      setSubmitError(
        error instanceof ApiError ? error.message : 'Unable to create AI task.',
      );
    }
  });
                ...
                <div className="mt-5 flex items-start gap-3 rounded-xl border border-red-500/20 bg-red-500/5 px-4 py-3 text-sm text-red-200">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-red-300/80">
                      {task.errorMessage ?? 'No error detail was returned.'}
                    </p>
                  </div>
                </div>
              ) : null}
